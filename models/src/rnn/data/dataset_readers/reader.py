...
def make_tokenizer(max_seq_len):
    def tokenizer(text): 
        return[Token(tok) for tok in j_t.tokenize(text, wakati=True)][:max_seq_len]
    return tokenizer

@DatasetReader.register("twireader")
class TwiReader(DatasetReader):
    """
    ...
    def __init__(
        ...
        self._tokenizer = make_tokenizer(max_sequence_length)
    ...
    def text_to_instance(
        ...
        if self._segment_sentences:
            ...
            for sentence in sentence_splits:
                word_tokens = self._tokenizer(sentence)
